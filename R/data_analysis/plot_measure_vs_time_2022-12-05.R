...
plt_df_subj_all <- readRDS(file.path(here(), "results_objects", "table_lmm_estimate_measure_vs_time_plt_df_subj_all.rds"))
plt_df_popul_all <- readRDS(file.path(here(), "results_objects", "table_lmm_estimate_measure_vs_time_plt_df_popul_all.rds"))
...
for (i in 1 : nrow(param_df)){ # i <- 1
  ...
  plt <- 
    ...
    geom_line(aes(x = x, y = mu), size = 0.5, linetype = 1, alpha =0.5) + 
...
plt_fpath <- file.path(here(), "results_figures", "lmm_estimate_measure_vs_time_plt_df_subj_all.jpeg")
